...
def drawDebugImage(image, final_results):
    for final_result in final_results:
        print(colored("final_result :{}".format(final_result), 'bright red'))
...
def deal_with_bodyAnalysis_result(img_bytes):
    ...
    if analysis_result["person_num"] > 0:
        ...
        for person_info in analysis_result['person_info']:
            ...
            person_data["nose"] = (round(body_parts["nose"]["x"]), round(body_parts["nose"]["y"]))
...
def distance_filter(results, depth_image):
    ...
    for result in results:
        coordinate = depth_image[int(result['location']['top'] + result['location']['height'] / 2)][
            int(result['location']['left'] + result['location']['width'] / 2)]
...
# 根据request查找符合条件的人
def find_person_by_request(results, target_index):
    if target_index == -1:
        return results[0]
    if target_index == 0:
        for result in results:
...
def generate_description(result):
    if result['gender'] == '男性':
        desc = "He is {glasses}wearing glasses. The main color of his upper clothe is {color}. He is {mask}wearing a face mask. He is {headwear}wearing a hat."
    else:
        desc = "She is {glasses}wearing glasses. The main color of her upper clothe is {color}. She is {mask}wearing a face mask. She is {headwear}wearing a hat."
    desc = desc.format(glasses='' if result['glasses'] == '戴眼镜' else 'not ',
                       color=clothes_color[result['upper_color']],
...
def call_back(req):
    ...
    while True:
        ...
        final_results = distance_filter(integrate_results(attr_result, analysis_result),
                                        capture.transformed_depth_point_cloud)
        print("Detected People Number is {}.".format(len(final_results)))
        if len(final_results) == 0:
            continue
        print(colored('peoples:{}'.format(final_results), 'bright red'))
        ...
        result = find_person_by_request(final_results, target_index)
        if result:
            print(colored(result, 'purple'))
            description = generate_description(result)
            coordinate = capture.transformed_depth_point_cloud[result['nose'][1] + random.randint(0, 2),
                                                               result['nose'][0] + random.randint(0, 2)]
            print(colored("Before transformed coordinate:{}".format(coordinate)))
            coordinate = [v / 1000 for v in coordinate]
            if coordinate[0] == coordinate[1] == coordinate[2]:
                print(colored("No depth information!", 'bright red'))
                continue
...
if __name__ == "__main__":
    ...
    camera = PyK4A(config)
    camera.start()
    rospy.init_node('findPeopleNode')
    service = rospy.Service('findPeople', xm_find_people, call_back)
    rospy.loginfo('GPSR Find Person\'s Vision Server Start!')
    rospy.spin()
