...
def are_you_sure(old_person, new_person):
    print("Ratio debug info is {}m.".format(
        (old_person[2][2] * old_person[2][3]) / (new_person[2][2] * new_person[2][3])))
    if (old_person[2][2] * old_person[2][3]) / (new_person[2][2] * new_person[2][3]) > 2 or \
            (old_person[2][2] * old_person[2][3]) / (new_person[2][2] * new_person[2][3]) < 0.5:
        return False
    return True
...
def people_tracking_publisher():
    ...
    turning_right = False
    turning_left = False
    old_coordinate = [-9000, -9000, -9000]
    cv2.namedWindow("follow", 0)
    cv2.resizeWindow("follow", 640, 480)
    cv2.moveWindow("follow", 1280, 600)
    while not rospy.is_shutdown():
        ...
        pose_xyz.position.header.stamp = rospy.Time(0)
