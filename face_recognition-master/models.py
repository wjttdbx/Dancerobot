...
def Linear(data, num_filter=1, kernel=(1, 1), stride=(1, 1), pad=0, num_group=1, name=None, suffix=''):
    ...
    if num_group == 1:
        ...
        out = tf.keras.layers.DepthwiseConv2D(kernel, stride, use_bias=False,
                                              depth_multiplier=depth_multiplier, name="%s%s_conv2d" % (name, suffix))(
            out)
...
def DResidual(data, num_out=1, kernel=(3, 3), stride=(2, 2), pad=1, num_group=1, name=None, suffix=''):
    conv_sep = conv(data, num_group, kernel=(1, 1), stride=(1, 1), pad=0, num_group=1,
    ...
    proj = Linear(conv_dw, num_out, kernel=(1, 1), stride=(1, 1), pad=0, num_group=1,
...
def Residual(data, num_block=1, num_out=1, kernel=(3, 3), stride=(1, 1), pad=1, num_group=1, name=None, suffix=''):
    ...
    for i in range(num_block):
        shortcut = identity
        out = DResidual(identity, num_out, kernel, stride, pad, num_group, name='%s%s_block' % (name, suffix),
                        suffix='%d' % i)
...
def get_model(image_w, image_h, pretrained=False):
    ...
    conv_3 = Residual(conv_23, num_block=4, num_out=64, kernel=(3, 3), stride=(1, 1), pad=1, num_group=128,
                      name="res_3")
    conv_34 = DResidual(conv_3, num_out=128, kernel=(3, 3), stride=(2, 2), pad=1, num_group=256, name="dconv_34")
    conv_4 = Residual(conv_34, num_block=6, num_out=128, kernel=(3, 3), stride=(1, 1), pad=1, num_group=256,
                      name="res_4")
    conv_45 = DResidual(conv_4, num_out=128, kernel=(3, 3), stride=(2, 2), pad=1, num_group=512, name="dconv_45")
    conv_5 = Residual(conv_45, num_block=2, num_out=128, kernel=(3, 3), stride=(1, 1), pad=1, num_group=256,
                      name="res_5")

    conv_6_sep = conv(conv_5, num_filter=512, kernel=(1, 1), pad=0, stride=(1, 1), name="conv_6sep")
    conv_6_dw = Linear(conv_6_sep, num_filter=512, kernel=(7, 7), stride=(1, 1), pad=0, num_group=512,
                       name="conv_6dw7_7")
    ...
    if pretrained:
        load_weights(r"C:\Users\wjttdbx\Desktop\xm_vision\xm_vision\src\scripts\face_recognition-master\weights"
                     r"\mobilefacenet.npy",
                     model)
    return model
